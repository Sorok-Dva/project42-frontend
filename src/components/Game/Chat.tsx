...
const Chat: React.FC<ChatProps> = ({
  ...
  // Effet pour faire défiler vers le bas lors du chargement initial
  useEffect(() => {
    scrollToBottom()
  }, [])
  ...
  return (
    <motion.div
      className="flex flex-col h-full bg-gradient-to-r from-black/60 to-blue-900/20 backdrop-blur-sm rounded-xl border border-blue-500/30 overflow-hidden"
    ...
    >
      {/* En-tête */ }
      <div
        className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 px-4 py-3 border-b border-blue-500/30 flex items-center justify-between">
      ...
      {/* Zone de messages */ }
      <div
        className="flex-1 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-500/30 scrollbar-track-black/20"
        ref={ chatMessagesRef }
      >
        {/* Règles du jeu */ }
        <div
      ...
      {/* Zone de saisie */ }
      { !isArchive &&
        (player || viewer?.user?.id || (!player
          && ['SuperAdmin', 'Admin', 'Developer', 'Moderator', 'ModeratorTest'].includes(userRole as string))) && (
        <div
          className="relative p-3 border-t border-blue-500/30 bg-black/40">
          <div className="flex">
            <input
              type="text"
          ...
          { suggestions.length > 0 && (
            <div
              className="sound-tick absolute bottom-full left-0 w-full bg-black/80 border border-blue-500/30 rounded-t-lg max-h-40 overflow-y-auto z-10">
              { suggestions.map((nickname, index) => (
                <div
                  key={ index }
                  className={ `px-4 py-2 cursor-pointer ${ selectedIndex === index ? 'bg-blue-900/50': 'hover:bg-blue-900/30' }` }
