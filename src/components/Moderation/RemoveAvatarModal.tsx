...
const RemoveAvatarModal: React.FC<RemoveAvatarModalProps> = ({ isOpen, onClose, targetUser }) => {
  const { token } = useAuth()
  const authHeaders = token ? { headers: { Authorization: `Bearer ${token}` } } : {}

  const handleRemove = async () => {
    try {
      await axios.post('/api/mod/remove-avatar', { userId: targetUser.id }, authHeaders)
    } catch (error) {
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-white">Supprimer l’avatar ?</h2>
              <button
                onClick={onClose}
            ...
            <p className="text-sm text-gray-300 mb-4">
              Voulez-vous vraiment supprimer l’avatar de ce joueur ?
            </p>
